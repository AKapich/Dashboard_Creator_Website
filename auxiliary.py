...
def get_starting_XI(match_id, team):
    ...
    startingXI = [
        p["player"]["name"]
        for p in events[events["type"] == "Starting XI"]["tactics"].values[0]["lineup"]
    ]
    early_replacements = events[events["substitution_replacement"].notna()][
        events["minute"] < 30
    ]
    early_replacements_dict = dict(
        zip(
            early_replacements["player"], early_replacements["substitution_replacement"]
        )
    )
    startingXI = [
        p if p not in early_replacements_dict.keys() else early_replacements_dict[p]
        for p in startingXI
    ]
    return startingXI
...
def get_players_xT(match_id):
    ...
    def get_xT(type):
        ...
        df[f"start_{type.lower()}_x_bin"] = pd.cut(
            df["start_x"], bins=xT_cols, labels=False
        )
        df[f"start_{type.lower()}_y_bin"] = pd.cut(
            df["start_y"], bins=xT_rows, labels=False
        )
        df[f"end_{type.lower()}_x_bin"] = pd.cut(
            df["end_x"], bins=xT_cols, labels=False
        )
        df[f"end_{type.lower()}_y_bin"] = pd.cut(
            df["end_x"], bins=xT_rows, labels=False
        )
        df["start_zone_value"] = df[
            [f"start_{type.lower()}_x_bin", f"start_{type.lower()}_y_bin"]
        ].apply(lambda z: xT[z[1]][z[0]], axis=1)
        df["end_zone_value"] = df[
            [f"end_{type.lower()}_x_bin", f"end_{type.lower()}_y_bin"]
        ].apply(lambda z: xT[z[1]][z[0]], axis=1)
