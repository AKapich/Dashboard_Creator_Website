    ...
    with threading.Lock():
        try:
            ...
            home_team_text.set_bbox(
                dict(
                    facecolor=country_colors[home_team],
                    alpha=0.5,
                    edgecolor="white",
                    boxstyle="round",
                )
            )
            away_team_text = axes[1].text(
                0.8,
                0.4,
                away_team,
                fontsize=30,
            ...
            away_team_text.set_bbox(
                dict(
                    facecolor=country_colors[away_team],
                    alpha=0.5,
                    edgecolor="white",
                    boxstyle="round",
                )
            ...
            return (
                jsonify({"error": "An error occurred while generating the plot"}),
                500,
            )
    ...
    with threading.Lock():
        try:
            ...
            if column == 1:
                ...
                viz_dict[plot_type](
                    match_id=match_id, home_team=home_team, away_team=away_team, ax=ax
                )
            else:
                viz_dict[plot_type](
                    match_id=match_id, team=away_team, ax=ax, inverse=True
                )
...
def save_dashboard():
    try:
        ...
        gs = fig.add_gridspec(
            nrows=n_rows, ncols=3, height_ratios=height_ratios, width_ratios=[1, 1, 1]
        )
        ...
        for p in range(0, len(dashboard)):
            if dashboard[p] != "None":
                ...
                if j == 0:
                    ...
                    viz_dict[dashboard[p]["plotType"]](
                        match_id, home_team, away_team, axes[i][j]
                    )
                elif j == 2:
                    viz_dict[dashboard[p]["plotType"]](
                        match_id, away_team, axes[i][j], inverse=True
                    )
