...
from auxiliary import (
    fetch_match_data,
    fetch_match_pass_data,
    fetch_match_shot_data,
    fetch_match_split_data,
)
...
def overview(match_id, home_team, away_team, ax):
    ...
    def get_basic_data(team, events):
        ...
        SoT = len(
            shots[
                ~(shots["shot_outcome"].isin(["Blocked", "Off T", "Post", "Wayward"]))
            ]
        )
...
def voronoi(match_id, home_team, away_team, ax):
    ...
    df = df[
        (df["minute"] < min_threshold)
        | ((df["minute"] == min_threshold) & (df["second"] < sec_threshold))
    ]
    ...
    pitch.polygon(
        team1, ax=ax, fc=country_colors[home_team], ec="white", lw=3, alpha=0.5
    )
    pitch.polygon(
        team2, ax=ax, fc=country_colors[away_team], ec="white", lw=3, alpha=0.5
    )

    # Plot players
    for i in range(len(df["x"])):
        pitch.scatter(
            df["x"][i], df["y"][i], ax=ax, color=country_colors[df["team"][i]]
        )
...
def pressure_heatmap(match_id, team, ax, inverse=False):
    ...
    pitch = Pitch(
        pitch_type="statsbomb", pitch_color="#0e1117", line_color="white", line_zorder=2
    )
    pitch.draw(ax=ax)

    bin_statistic = pitch.bin_statistic(
        press_df.x, press_df.y, statistic="count", bins=(8, 6), normalize=False
    )
    pitch.heatmap(
        bin_statistic,
        edgecolor="#323b49",
        ...
        cmap=LinearSegmentedColormap.from_list(
            "custom_cmap", ["#f3f9ff", country_colors[team]], N=100
        ),
    )
...
def passing_network(match_id, team, ax, inverse=False):
    ...
    passes = passes[
        passes["player"].isin(startingXI) & passes["recipient"].isin(startingXI)
    ]
    ...
    average_location = passes.groupby("player").agg(
        {"x": ["mean"], "y": ["mean", "count"]}
    )
    ...
    passes_between = passes_between.merge(
        average_location, left_on="recipient", right_index=True, suffixes=("", "_end")
    )
    ...
    pitch.arrows(
        ...
        color="#d4d4d4",
        alpha=(pd.to_numeric(passes_between["pass_count"], downcast="float") / 20).clip(
            upper=1
        ),
        ax=ax,
    )
...
def progressive_passes(match_id, team, ax, inverse=False):
    ...
    if not inverse:
        df["beginning"] = np.sqrt(
            np.square(120 - df["start_x"]) + np.square(80 - df["start_y"])
        )
...
def penalty_passes(match_id, team, ax, inverse=False):
    ...
    df["to_penalty"] = (
        ...
        if not inverse
        else (df["end_x"].between(0, 18) & df["end_y"].between(18, 62))
    )
...
def shot_types(match_id, home_team, away_team, ax):
    ...
    pitch = VerticalPitch(
        pitch_type="statsbomb", pitch_color="#0e1117", line_color="#c7d5cc"
    )
    pitch.draw(ax=ax)

    shots = fetch_match_shot_data(match_id)

    shots[["start_x", "start_y"]] = pd.DataFrame(
...
def xG_flow(match_id, home_team, away_team, ax):
    ...
    for _, row in goals.iterrows():
        ...
        if row["team"] == away_team:
            a_goals.append(
                (row["minute"], a_xG[a_min.index(row["minute"])], annotation_text)
            )
        else:
            h_goals.append(
                (row["minute"], h_xG[h_min.index(row["minute"])], annotation_text)
            )
    ...
    ax.scatter(
        [i[0] for i in a_goals],
        [i[1] for i in a_goals],
        color=country_colors[away_team],
        zorder=10,
        s=250,
        marker="h",
    ...
    ax.scatter(
        [i[0] for i in h_goals],
        [i[1] for i in h_goals],
        color=country_colors[home_team],
        zorder=10,
        s=250,
        marker="h",
    ...
    for _, row in events.query('type == "Own Goal Against"').iterrows():
        color = (
            ...
            if row["team"] == away_team
            else country_colors[away_team]
        )
        for_team = home_team if row["team"] == away_team else away_team
        y = row["minute"]
        x = shots[shots["minute"] < row["minute"]][shots["team"] == for_team][
            "xG"
        ].sum()
...
def shot_xg(match_id, team, ax, inverse=False):
    ...
    pitch = VerticalPitch(
        pitch_type="statsbomb", pitch_color="#0e1117", line_color="#c7d5cc", half=True
    )
    pitch.draw(ax=ax)
...
def pass_heatmap(match_id, team, ax, inverse=False):
    ...
    sns.kdeplot(
        ...
        cmap=LinearSegmentedColormap.from_list(
            "",
            [lighten_hex_color(country_colors[team], 0.45), country_colors[team]],
            N=100,
        ),
        ax=ax,
    )
...
def xT_scatterplot(match_id, home_team, away_team, ax):
    ...
    top_carry_xT = (
        xtdf.sort_values("carry_xT", ascending=False).head(5)["player"].values
    )
    ...
    for _, row in xtdf.iterrows():
        ...
        if (
            row["player"] in top_pass_xT
            or row["player"] in top_carry_xT
            or row["player"] in top_xT
        ):
            if row["player"] not in annotation_fix_dict.keys():
                annotation_text = row["player"].split(" ")[-1]
            else:
...
def xT_heatmap(match_id, team, ax, inverse=False):
    ...
    pitch.heatmap(
        ...
        alpha=0.65,
        cmap=LinearSegmentedColormap.from_list(
            "", ["#f3f9ff", darken_hex_color(country_colors[team], 0.3)], N=20
        ),
    )
    ...
    ax.set_title(
        f"{team} xT Pass+Carry (Start Zones)",
        color="white",
        fontsize=20,
        fontweight="bold",
        fontfamily="Monospace",
...
def xT_momentum(match_id, home_team, away_team, ax):
    ...
    xT_data = pd.concat(
        [
            get_xT(events=df, type="Pass", momentum=True),
            get_xT(events=df, type="Carry", momentum=True),
        ],
        axis=0,
    )
    ...
    goals = df[(df["shot_outcome"] == "Goal") | (df["type"] == "Own Goal For")][
        ["minute", "team"]
    ]
